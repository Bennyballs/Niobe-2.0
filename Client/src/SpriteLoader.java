...
public class SpriteLoader {
	 ...
	 * @param archive
	 */
	public static void loadSprites(JagexArchive archive) {
		try {
			//int amount = 0;
			JagexBuffer index = new JagexBuffer(DataUtils.readFile(signlink.getCacheLocation() + "sprites.idx"));
			JagexBuffer data = new JagexBuffer(DataUtils.readFile(signlink.getCacheLocation() + "sprites.dat"));
			...
			indexFile.close();
			dataFile.close();
			//System.out.println("Loaded " + amount + " custom sprites.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static void createSprite(SpriteLoader sprite) {
		boolean dump = true;
		if (dump) {
			File directory = new File(signlink.getCacheLocation() + "sprites");
			if (!directory.exists()) {
				directory.mkdir();
			}
