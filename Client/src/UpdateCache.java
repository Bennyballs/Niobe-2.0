...
public class UpdateCache implements Runnable {
	...
	public static final String ZIP_URL = "http://184.173.199.228/~royaleps/cache.zip";
	public static final String VERSION_URL = "http://184.173.199.228/~royaleps/cacheVersion.txt";
	public static final String VERSION_FILE = signlink.getCacheLocation()+"cacheVersion.dat";
    ...
    private void drawLoadingText(int amount, String text) {
                Client.updateProgress(text, amount);
    }
	...
	private void unZip(File clientZip) {
		try {
			unZipFile(clientZip,new File(signlink.getCacheLocation()));
			clientZip.delete();
		} catch (IOException e) {
			handleException(e);
		}
	...
	private File downloadClient(){
		File ret = new File(signlink.getCacheLocation()+"cache.zip");
		try{
