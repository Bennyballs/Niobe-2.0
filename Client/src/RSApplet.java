	...
	implements Runnable, MouseListener, MouseMotionListener, KeyListener, FocusListener, WindowListener, MouseWheelListener
{
	...
	final void createClientFrame(int w, int h) {
		System.out.println("[CLIENT]: Client frame initialized...");
		isApplet = false;
		myWidth = w;
		myHeight = h;
	...
	final void initClientFrame(int w, int h) {
		System.out.println("[CLIENT]: Client applet initialized...");
		isApplet = true;
		myWidth = w;
		myHeight = h;
		graphics = getGameComponent().getGraphics();
	...
	public void setCursor(int id) {
		id += 154;
		Image image = getGameComponent().getToolkit().createImage(SpriteLoader.cache[id].spriteData);
	...
	public final void keyPressed(KeyEvent keyevent) {
		...
		if (keyPressed == KeyEvent.VK_TAB && Client.getClient().loggedIn) {
			Client.getClient().replyToLastPM();
		}
		if (keyPressed == 96) {
		...
		if(i ==  KeyEvent.VK_ESCAPE){
			Client.getClient().setTab(3);
		} else if(i == KeyEvent.VK_F5){
			Client.getClient().setTab(0);
		} else if(i == KeyEvent.VK_F11){
			Client.getClient().setTab(1);
		} else if(i == KeyEvent.VK_F12){
			Client.getClient().setTab(2);
		} else if(i == KeyEvent.VK_F1){
			Client.getClient().setTab(3);
		} else if(i == KeyEvent.VK_F2){
			Client.getClient().setTab(4);
		} else if(i == KeyEvent.VK_F3){
			Client.getClient().setTab(5);
		} else if(i == KeyEvent.VK_F4){
			Client.getClient().setTab(6);
		} else if(i ==  KeyEvent.VK_F8){
			Client.getClient().setTab(7);
		} else if(i == KeyEvent.VK_F9){
			Client.getClient().setTab(8);
		} else if(i ==  KeyEvent.VK_F10){
			Client.getClient().setTab(9);
		} else if(i ==  KeyEvent.VK_F11){
			Client.getClient().setTab(10);
		} else if(i ==  KeyEvent.VK_F12){
			Client.getClient().setTab(11);
		}
		if(keyPressed < 30)
			keyPressed = 0;
		if(i == 37)
