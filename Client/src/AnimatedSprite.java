...
public final class AnimatedSprite extends Sprite
{
	...
	public static String location = signlink.getCacheLocation() + "/rsimg/";

	public AnimatedSprite(String file)
	{
		this(location + file, DUMMY_COMPONENT);
	}
	...
	@Override
	public void renderRGBPixels(int srcPtr, int scanSize, int[] dst, int[] src, int srcOff, int height, int dstOff, int alpha, int dstPtr)
	{
		int scanSizeHigh = -(scanSize >> 2);
		scanSize = -(scanSize & 3);
		int i;
