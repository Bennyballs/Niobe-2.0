...
public class JagexArchive {
	...
	public JagexArchive(byte abyte0[]) {
		...
		int i = stream.readTripleBytes();
		int j = stream.readTripleBytes();
		if (j == 0) {
			...
			stream = new JagexBuffer(aByteArray726);
			aBoolean732 = true;
		} else if (j != i) {
			byte abyte1[] = new byte[i];
		...
		for (int l = 0; l < dataSize; l++) {
			...
			anIntArray729[l] = stream.readTripleBytes();
			anIntArray730[l] = stream.readTripleBytes();
			anIntArray731[l] = k;
			k += anIntArray730[l];
		}
	}
