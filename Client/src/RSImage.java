...
public final class RSImage extends DrawingArea {
	...
	public static String location = signlink.getCacheLocation() + "rsimg" + System.getProperty("file.separator");
	...
	public RSImage(JagexArchive archive, String spriteArchive, int spriteIndex) {
		...
		indexBuffer.currentOffset = dataBuffer.readUnsignedShort();
		...
		for (int index = 0; index < total - 1; index++) {
			pixels[index + 1] = indexBuffer.readTripleBytes();
			if (pixels[index + 1] == 0) {
				pixels[index + 1] = 1;
			}
		}
		for (int index = 0; index < spriteIndex; index++) {
			...
			dataBuffer.currentOffset += indexBuffer.readUnsignedShort() * indexBuffer.readUnsignedShort();
			indexBuffer.currentOffset++;
		}
		offsetX = indexBuffer.readUnsignedByte();
		offsetY = indexBuffer.readUnsignedByte();
		myWidth = indexBuffer.readUnsignedShort();
		myHeight = indexBuffer.readUnsignedShort();
